...
- Physical bounded-context partitioning is implemented in a single database using schemas (`sales_ordering`, `catalog`, `crm`, `fulfillment`, `sales_org`, `purchasing`, `reporting`).
...
| `FK_Products_Suppliers` | `catalog.products.supplier_id` | `purchasing.suppliers.supplier_id` | Product Catalog | Purchasing | Planned for phased decoupling |
...
- **Product Catalog** references Purchasing (`SupplierId`).
...
- Product Catalog, CRM, Sales Organization, Fulfillment, Purchasing: **Upstream** master data providers.
...
- `Purchasing -> Product Catalog` (`SupplierId` reference)
...
- `Purchasing -> Reporting` (`Supplier*` events)
