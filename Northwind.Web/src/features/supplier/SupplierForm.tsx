...
import type { SupplierDetails, SupplierRequest } from "./types";
...
interface SupplierFormProps {
  ...
  isSaving: boolean;
  error: string | null;
  onSave: (request: SupplierRequest) => void;
  onClose: () => void;
}

const empty: SupplierRequest = {
  companyName: "",
...
function toRequest(s: SupplierDetails): SupplierRequest {
  return {
...
export function SupplierForm({
  ...
  useEffect(() => {
    setForm(supplier ? toRequest(supplier) : empty);
  }, [supplier]);
  ...
  const set = (field: string, value: string) => {
    ...
    setForm((prev) => {
      ...
      if (field === "phone" || field === "fax" || field === "homepageUrl")
        return {
          ...prev,
          communication: { ...prev.communication!, [field]: val },
        };
          ...
          >
            <div className="flex-1 space-y-4 px-6 py-4">
              ...
              <fieldset className="space-y-3">
                ...
                <Field
                  label="Phone"
                  value={form.communication?.phone ?? ""}
                  onChange={(v) => set("phone", v)}
                />
                <Field
                  label="Fax"
                  value={form.communication?.fax ?? ""}
                  onChange={(v) => set("fax", v)}
                />
                <Field
                  label="Home Page"
                  value={form.communication?.homepageUrl ?? ""}
