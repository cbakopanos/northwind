...
-- Keep identity in sync after explicit supplier_id inserts
SELECT setval(
    pg_get_serial_sequence('supplier.suppliers', 'supplier_id'),
    (SELECT MAX(supplier_id) FROM supplier.suppliers)
);
...
SELECT setval(
    pg_get_serial_sequence('catalog.categories', 'category_id'),
    (SELECT MAX(category_id) FROM catalog.categories)
);
...
SELECT setval(
    pg_get_serial_sequence('catalog.products', 'product_id'),
    (SELECT MAX(product_id) FROM catalog.products)
);
