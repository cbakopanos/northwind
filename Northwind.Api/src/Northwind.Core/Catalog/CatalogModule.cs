...
public sealed class CatalogModule : IModule
{
    ...
    {
        endpoints.MapGet("/api/catalog/health", async (ICategoriesRepository categories, IProductsRepository products,
        ...
        endpoints.MapGet(
                ...
                async (ICategoriesRepository query, ILogger<CatalogModule> logger,
        ...
        endpoints.MapGet(
                ...
                async (int categoryId, ICategoriesRepository query, ILogger<CatalogModule> logger,
                    CancellationToken cancellationToken) =>
                {
                    logger.LogInformation("Handling {Endpoint} for CategoryId {CategoryId}",
                        "GET /api/catalog/categories/{categoryId:int}", categoryId);
        ...
        endpoints.MapGet(
                ...
                {
                    logger.LogInformation("Handling {Endpoint} for CategoryId {CategoryId}",
                        "GET /api/catalog/categories/{categoryId:int}/picture", categoryId);
        ...
        endpoints.MapPost(
                "/api/catalog/categories",
                async (CategoryRequest request, ICategoriesRepository repository, ILogger<CatalogModule> logger,
                    CancellationToken cancellationToken) =>
                {
        ...
        endpoints.MapPut(
                ...
                {
                    logger.LogInformation("Handling {Endpoint} for CategoryId {CategoryId}",
                        "PUT /api/catalog/categories/{categoryId:int}", categoryId);

                    if (string.IsNullOrWhiteSpace(request.CategoryName))
                    {
                        logger.LogInformation(
                            "Update category request rejected for CategoryId {CategoryId}: CategoryName is required",
                            categoryId);
                        return Results.BadRequest(new { error = "CategoryName is required." });
                    }
        ...
        endpoints.MapGet(
                    ...
                    CancellationToken cancellationToken) =>
                {
                    ...
                    logger.LogInformation("Handling {Endpoint} (page {Page}, pageSize {PageSize})",
                        "GET /api/catalog/products", currentPage, currentPageSize);
        ...
        endpoints.MapGet(
                ...
                async (int productId, IProductsRepository query, ILogger<CatalogModule> logger,
                    CancellationToken cancellationToken) =>
                {
                    logger.LogInformation("Handling {Endpoint} for ProductId {ProductId}",
                        "GET /api/catalog/products/{productId:int}", productId);
        ...
        endpoints.MapPost(
                "/api/catalog/products",
                async (ProductRequest request, IProductsRepository repository, ILogger<CatalogModule> logger,
                    CancellationToken cancellationToken) =>
                {
        ...
        endpoints.MapPut(
                ...
                {
                    logger.LogInformation("Handling {Endpoint} for ProductId {ProductId}",
                        "PUT /api/catalog/products/{productId:int}", productId);

                    if (string.IsNullOrWhiteSpace(request.ProductName))
                    {
                        logger.LogInformation(
                            "Update product request rejected for ProductId {ProductId}: ProductName is required",
                            productId);
                        return Results.BadRequest(new { error = "ProductName is required." });
                    }
        ...
        endpoints.MapPatch(
                ...
                {
                    logger.LogInformation("Handling {Endpoint} for ProductId {ProductId}",
                        "PATCH /api/catalog/products/{productId:int}/discontinue", productId);
        ...
        endpoints.MapPatch(
                ...
                {
                    logger.LogInformation("Handling {Endpoint} for ProductId {ProductId}",
                        "PATCH /api/catalog/products/{productId:int}/reinstate", productId);
