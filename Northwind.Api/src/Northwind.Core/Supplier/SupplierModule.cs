...
public sealed class SupplierModule : IModule
{
    ...
    {
        endpoints.MapGet("/api/supplier/health", async (ISuppliersRepository repository, ILogger<SupplierModule> logger,
            CancellationToken cancellationToken) =>
        {
            ...
            return Results.Ok(new { context = "Supplier", status = "ok", count });
        });

        endpoints.MapGet(
                "/api/supplier/suppliers",
                async (int? page, int? pageSize, ISuppliersRepository query, ILogger<SupplierModule> logger,
                    CancellationToken cancellationToken) =>
                {
                    var currentPage = page ?? 1;
                    var currentPageSize = Math.Clamp(pageSize ?? 10, 1, 100);
                    logger.LogInformation("Handling {Endpoint} (page {Page}, pageSize {PageSize})",
                        "GET /api/supplier/suppliers", currentPage, currentPageSize);
        ...
        endpoints.MapGet(
                ...
                async (int supplierId, ISuppliersRepository query, ILogger<SupplierModule> logger,
                    CancellationToken cancellationToken) =>
                {
                    logger.LogInformation("Handling {Endpoint} for SupplierId {SupplierId}",
                        "GET /api/supplier/suppliers/{supplierId:int}", supplierId);
        ...
        endpoints.MapPost(
                "/api/supplier/suppliers",
                async (SupplierRequest request, ISuppliersRepository repository, ILogger<SupplierModule> logger,
                    CancellationToken cancellationToken) =>
                {
        ...
        endpoints.MapPut(
                ...
                async (int supplierId, SupplierRequest request, ISuppliersRepository repository,
                    ILogger<SupplierModule> logger, CancellationToken cancellationToken) =>
                {
                    logger.LogInformation("Handling {Endpoint} for SupplierId {SupplierId}",
                        "PUT /api/supplier/suppliers/{supplierId:int}", supplierId);

                    if (string.IsNullOrWhiteSpace(request.CompanyName))
                    {
                        logger.LogInformation(
                            "Update supplier request rejected for SupplierId {SupplierId}: CompanyName is required",
                            supplierId);
                        return Results.BadRequest(new { error = "CompanyName is required." });
                    }
